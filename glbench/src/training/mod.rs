...
//!
//! # glbench observes; gltrain drives
//!
//! Everything here is downstream of a run gltrain controls. [`runner`]
...
//!
//! gltrain's `Backend` is not dyn-compatible (KL-001), so a naive observer API
...
//!
//! gltrain M2 trains one linear layer: no tokenizer, no batching beyond one
