...
**Status:** ✅ Done. Landed on `gltrain-m1` (82f244b, eafab0b), pushed to `github`.

---

## Problem

`gltrain/` was growing its own type vocabulary (`Tensor`, `Tape`, `GlProc`,
...
`gl-agent-skills/gltrain-naming/SKILL.md`, mirrored into `.claude/skills/` for
...
- **The prefix convention has 0 of 224 public types adopted** across gljax,
  glcore, glproc, glcuda, glserve, and gltrain. Both skills now open with
...
- gltrain's "current type map" was a rename target, not a map — nothing is
  renamed yet; `gltrain/src/lib.rs:18-21` still re-exports `Tensor`, `Tape`,
  `GlProc`, `SisdBackend`.
...
(Kevrin/Karg/Kevskrid/Gwellaer) every gltrain module header already carries
...
`git log --oneline -3` on `gltrain-m1` shows 82f244b (skills) and eafab0b
...
gltrain-m1` matches local `HEAD` at the time.
