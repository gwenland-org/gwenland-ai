...
> **Applies to:** `gltrain/src/checkpoint/` (does not exist yet)
> **Scope:** `CPLora` (**FULL**), `CPFull`, `CPSharded`, `CPIncremental` (all
> **STUB**, real formats), `PLGgufMerge` (**STUB**, separate trait)
> **Prerequisite reading:** `gltrain/M2_RESEARCH.md` §3, §7-D, §7-E, §8.5 — the
...
3. **`TensorId` cannot be a checkpoint key.** `gltrain/src/tensor/tensor.rs`'s
...
10. **`CPFull` (stub) — same segments, all parameters instead of just the
    ...
    gltrain has no model tree yet to enumerate ("all params" requires a
...
- [../gltrain-naming/SKILL.md](../gltrain-naming/SKILL.md) — the `CP`/`PL` prefix decisions and full M2 name map
