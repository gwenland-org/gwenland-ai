...
> **Applies to:** `stumman/src/optim/` (does not exist yet)
> **Scope:** `OPAdamW` (**FULL**), `OPLion`, `OPAdafactor`, `OPAdamW8bit` (all
> **STUB**, with real state shapes)
> **Prerequisite reading:** `stumman/M2_RESEARCH.md` §4, §7-B, §7-E, §8.3 — the
> full research and architecture record this skill distills.
> **Last updated:** 2026-08-17

## BEFORE YOU START

- [ ] I have read `stumman/KNOWN_ISSUES.md` KL-005 and KL-006. KL-006 is not
...
- [ ] The optimizers' **helper** types are prefixed too: `OPAdamWMoments` (not
      ...
      `stumman-naming/SKILL.md`), and the Adafactor one takes `EN` because a
      closed set of variants is its whole job. Landed M2 Wave 3.
- [ ] I have read `stumman/src/nn/param.rs` (`TPParameter`) and
      `stumman/src/nn/module.rs` (`Module`, `trainable_parameters`) — the
      optimizer updates these, it does not invent its own parameter type.
- [ ] I have read `stumman/src/autograd/tape.rs`'s `Tape::finish_step` — the
...
1. **The exact update, decoupled weight decay, same-step form** (verified
   ...
   *after* the adaptive step (`θ *= (1 - lr·wd)`) is a real bug that appeared in
   this crate's own planning doc (`STUMMAN_PLAN.md` §3.4) — it leaves a spurious
...
- [../stumman-naming/SKILL.md](../stumman-naming/SKILL.md) — the `OP` prefix decision and full M2 name map
