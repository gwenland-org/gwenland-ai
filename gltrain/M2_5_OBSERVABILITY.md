...
**Status:** complete
**Branch:** `gltrain-m2.5-observability` (off `gltrain-m2`)
...
warnings from `gltrain` (two pre-existing `unused_mut` warnings come from
...
`gltrain` can name every type in the trait signature, implement `StepObserver`,
...
`gl-agent-skills/gltrain-naming/SKILL.md`'s Breton table lists seven sub-systems
...
`gltrain/` and this milestone's brief said to stop at the crate boundary. It is
