...
- patch: 362b36352d8de8e41d71bad4d86e6063fc41a5d4d99151a0b1e260fee217df50
- runs: 3 (medians), each counterbalanced forward and reverse
- parity gate: test result: ok. 31 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 11.74s
...
| shape | out x in | full us | no math | no barriers | no staging | no epilogue | GMAC/s | spread |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| `ffn_gate_up` | 9728x896 | 363.67 | 225.80 | 354.80 | 269.40 | 355.46 | 5848 | 1.6% |
| `ffn_down` | 896x4864 | 209.69 | 133.56 | 188.45 | 154.84 | 208.47 | 5071 | 0.6% |

### What each removal buys

| shape | floor (staging+barriers) | removing barriers | removing staging | **removing epilogue** |
|---|---:|---:|---:|---:|
| `ffn_gate_up` | 62.1% of the kernel | +2.4% | +25.9% | **+2.3%** |
| `ffn_down` | 63.7% of the kernel | +10.1% | +26.2% | **+0.6%** |
...
- gate_up saves **+2.4%**, down saves **+10.1%** -> ratio **4.15x** against 5.4x the barriers
- **STAGING IS THE WALL - 26% against 2% for the epilogue, so overlapping the load with the math (NumStages=2) is the lever.**
