...
- **Blocking gap found 2026-07-22, not yet built**: there is no end-to-end
  `.gllm` token-generation path. `glictus-caliburni::runtime::GllmRuntime`
  only has `NullBackend` (orchestration only — proves the runtime walks a
  real model, produces no logits; see `examples/run_package.rs`).
  `GlprocBackend` (`runtime/glproc_backend.rs`) exists and forwards
  `execute_layer` to glproc's kernels, but nothing drives it through a full
  forward pass + sampling + detokenize loop with a TPS counter. Every PPL/TPS
  number above requires this to exist first — it's the actual next task,
...
4. **[CRITICAL, open 2026-07-22 — one real bug fixed, root cause still NOT found] Real-model generation quality is unknown-bad, not unknown-good.** The first-ever `.gllm` E2E text generation test (`glictus-caliburni/examples/run_package_e2e.rs`, built to answer this) ran a real GQ4A_CPP package (qwen2.5-0.5b-instruct) through full prefill+decode and got **coherent-input, garbage-output** — multilingual noise tokens, not a plausible completion — despite the identical source GGUF producing correct output via the older `glcore`/`GlprocEngine` path in the same session. TPS was also ~17-20x below that baseline (1.0-1.4 tok/s vs 24-69 tok/s), with every layer apparently being unmapped and remapped from disk on every single token.
   **RMSNorm epsilon bug found, fixed, and verified NOT to be the (sole) cause.** RMSNorm epsilon was hardcoded `1e-5` independently in both `GlprocBackend` and `GllmEngine`, while this exact test model's GGUF declares `1e-6` (confirmed via `gwen info`) — a real 10x error across every norm call in every layer, mechanistically plausible as the cause. Fixed properly (not worked around): `ModelMetadata` gained an `rms_eps` field + `effective_rms_eps()` accessor, the ARTX07 converter now reads `{arch}.attention.layer_norm_rms_epsilon` from GGUF, and both runtime call sites use the real value. All 316 tests pass; the real model's manifest now correctly records `~1e-6`. **Re-ran the E2E test after the fix: output is still garbage** (a different garbage string, as expected since the norm math changed, but equally incoherent) — so this was a genuine bug worth fixing, but not what's causing the symptom, or not the only thing. Full detail in `notes/pridwen-p1-notes.md`'s 2026-07-22T08:15:00Z FINDING entry.
   Ruled out by the earlier point-by-point diff against the known-good `glproc::runner::Runner` path: attention scale-factor placement, RoPE arithmetic, GQA head-to-kv-head mapping, and weight-matrix transpose/layout are all byte-identical or structurally equivalent between the two paths. A related-but-separate real gap was also found and explicitly NOT the cause on this model: `head_dim` is derived only as `embedding_length/num_heads` in `.gllm`, never preferring GGUF's `attention.key_length` override the way `glproc`'s loader does — this GGUF has no `key_length` key, so both paths coincidentally agree here, but it remains an open correctness gap for other models, unfixed.
   **Not yet tried**: the differential logits/hidden-state dump (compare `Runner::step`'s raw output against `GllmEngine::logits_for`'s on the identical token, greedy sampling) that was recommended before the epsilon fix and still hasn't been built — this is very likely the fastest remaining way to actually localize the bug, since two rounds of code-reading-based hypothesis testing (this entry, the previous one) have each found a real-but-insufficient bug rather than the actual cause. Also not yet tried: isolating whether GQ4A quantization itself is implicated (no all-F32 real-model package currently constructible without code changes, since CPP always assigns `token_embd`/`output` to GQ4A even under the escape hatch — see item #5). Every PPL/TPS number anywhere in this document (§13, §14) remains untrustworthy until root cause is found and output is confirmed coherent.
5. **[Related to #4] `.gllm`'s runtime read path (`GllmEngine::load_shared`, `GlprocBackend::required_tensor`) cannot decode native GGUF quantized dtypes (Q4_K/Q5_0/Q8_0/Q6_K).** Only F32/F16/BF16 (glcore's `decode_tensor`) and now GQ4A/GQ2A (fixed 2026-07-22, dequant added directly at both call sites via glproc) are readable at runtime. Any `.gllm` package where CPP leaves a tensor at its original GGUF quant dtype — which is most tensors in a package converted with no `--quant` flag, and can include layers CPP doesn't recognize even with one — fails to load. The *converter* already has glproc-backed Q4_K/Q5_0 dequant (the Phase 2 ADR), but only applies it when re-encoding a tensor to GQ4A/GQ2A, not when passing a tensor through unchanged. Extending `.gllm`'s runtime read path to cover every GGUF quant dtype glproc already supports (mirroring the ADR's precedent) is unstarted work, separate from item #4.
