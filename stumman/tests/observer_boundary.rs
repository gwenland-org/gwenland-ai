...
//!
//! An integration test links `stumman` as an **external crate**, so it can only
...
//! would fail to compile and nothing in stumman's own suite would notice.
//!
...
use stumman::autograd::grad_store::VLGradStore;
use stumman::backend::GlProc;
use stumman::optim::VLNamedTensor;
use stumman::train::observe::{StepObserver, VLTrainingStep};
...
/// the boundary, not the numerics, and stumman's own suite covers those.
const D_IN: usize = 4;
