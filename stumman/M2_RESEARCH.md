...
> **Baseline measured:** `cargo test --lib` in `stumman/` → **62 passed, 0 failed**
...
### 0.1 "gltrain" is two different things, and the one that matters is `stumman/`

| | `gltrain/` | `stumman/` |
|---|---|---|
| Crate name | `gwenland-core` | `stumman` |
...
`stumman/STUMMAN_PLAN.md` is literally titled **"Stummañ — gltrain Planning
...
M2 in this task. **All work below targets `stumman/`.** `gltrain/` is treated
as prior art to read, not as a codebase to extend.

### 0.2 The plan's own M2 sketches are stale in three places

`STUMMAN_PLAN.md` was written before M1 existed. Verified against the tree:
...
1. **§3.8 says "Reuse existing LoRA logic from gltrain/src/train/lora.rs".**
   ...
   is nothing to reuse; the API it is built on does not exist in stumman.
...
### 1.1 What `stumman` actually is, as of M1
...
ML dependencies, no C bindings". stumman is the training arm and is allowed
...
Note the last row of prior art contradicts `STUMMAN_PLAN.md` Q8, which decided
...
- it needs a column-norm op and a detach-from-graph op that stumman lacks.
...
**Blocked below the adapter layer.** stumman has no conv op, no 4-D tensors, and
...
checkpointing for stumman would have to be value-delta + compression, not
...
The one thing that *is* easy to get wrong, and that `STUMMAN_PLAN.md` §3.4 gets
...
| Skill | Complexity | Memory impact | Dependencies (on stumman work) | M2 feasibility | Recommended abstraction |
|---|---|---:|---|---|---|
...
| Consumer | stumman itself | glproc / glictus-caliburni / llama.cpp |
...
need. `STUMMAN_PLAN.md` Q8's "no optimizer state in v1" then becomes a *default*
...
Every new file opens with its Breton sub-system header, per stumman-naming.
...
1. **`OP` prefix approval (blocking for `optim/`).** `stumman-naming/SKILL.md`:
...
3. **Confirm M2 targets `stumman/`, not `gltrain/`** (§0.1).
...
**In-repo** — `stumman/STUMMAN_PLAN.md`, `stumman/KNOWN_ISSUES.md`,
