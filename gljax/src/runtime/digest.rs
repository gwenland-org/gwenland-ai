...
//! # Why hand-written
//!
//! ARTX01 §6.3 specifies SHA-256 for the cache key and ARTX01 §5.4 caps the
//! dependency list at `libloading` + `log` + `glcore`. `sha2` is already in the
//! workspace lockfile (via `glictus-caliburni`), so adding it would cost no
//! build time — but that is a dependency decision, and the sprint brief says to
//! declare one rather than take it. ~90 lines against the published test
//! vectors is the cheaper answer.
//!
//! ⭐ If a dependency is ever acceptable here, delete this file and call
//! `glictus_caliburni::sha256_bytes` — it is the same algorithm, audited.
...
pub fn sha256(data: &[u8]) -> [u8; 32] {
    ...
    // Message padding: 0x80, then zeros, then the bit length as a big-endian
    // u64, so the total is a multiple of 64 bytes.
