...
pub fn swiglu_ffn(x: &Tensor, gate_proj: &Tensor, up_proj: &Tensor, down_proj: &Tensor) -> Tensor {
    ...
    assert_eq!(
        gate_proj.shape().dims.first().copied(),
        Some(d),
        "swiglu_ffn: gate_proj must be [D, FFN] with D = {d}"
    );
    assert_eq!(
        up_proj.shape().dims,
    ...
    let ffn = gate_proj.dim(1);
    assert_eq!(
        down_proj.shape().dims,
        vec![ffn, d],
        "swiglu_ffn: down_proj must be [FFN, D] = [{ffn}, {d}]"
    );

    let gate = x.matmul(gate_proj).silu();
    let up = x.matmul(up_proj);
    gate.mul(&up).matmul(down_proj)
}
...
mod tests {
    ...
    fn swiglu_returns_to_the_model_dimension() {
        ...
        let gate = cx.weight("gate_proj.weight", Shape::new([896, 4864], DType::F32));
        let up = cx.weight("up_proj.weight", Shape::new([896, 4864], DType::F32));
        let down = cx.weight("down_proj.weight", Shape::new([4864, 896], DType::F32));
    ...
    fn silu_is_applied_to_the_gate_branch_not_the_up_branch() {
        ...
        let gate = cx.weight("g", Shape::new([4, 8], DType::F32));
        let up = cx.weight("u", Shape::new([4, 8], DType::F32));
    ...
    #[test]
    #[should_panic(expected = "down_proj must be [FFN, D]")]
