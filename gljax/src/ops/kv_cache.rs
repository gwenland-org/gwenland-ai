...
//! What is **not** here is a cache-aware Qwen2 trace: wiring the cache through
//! attention changes the compiled signature (two extra in/out parameters per
//! layer), needs buffer donation to avoid copying the cache every step, and
//! neither can be validated without a plugin. Landing it unmeasured would be
...
/// with the cache buffer donated (`input_output_alias`), XLA turns the
/// functional update into an in-place store. Without donation it *is* a full
/// copy of the cache every step, which at Qwen2-0.5B's 24 layers × 2048
/// positions is the difference between a working decode and an unusable one.
/// Donation is a `CompileOptions` field gljax does not set yet.
pub fn write_at(
    cache: &Tensor,
    update: &Tensor,
    layer: &Tensor,
    ...
    assert_eq!(
        update.dim(0),
        1,
        "write_at writes one layer at a time, got {}",
        update.dim(0)
    );
    assert_eq!(
        update.dim(1),
        1,
        "write_at writes one position at a time, got {}",
        update.dim(1)
    );
    assert_eq!(
