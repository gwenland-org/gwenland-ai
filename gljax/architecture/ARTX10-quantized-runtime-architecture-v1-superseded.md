...
>
> This document designs a quantized runtime around
> `stablehlo.uniform_dequantize → dot_general`. **That path does not exist.**
> Probed 2026-07-28 against the PJRT CPU plugin: the `quant` dialect is not
> even registered, so the module does not parse — see
> [`ARTX10-quantized-runtime-architecture.md`](ARTX10-quantized-runtime-architecture.md)
> §Q1 and [`../probes/pjrt_cpu_quant_probe.py`](../probes/pjrt_cpu_quant_probe.py).
>
> Kept because the emission-dispatch and capability-probe *shapes* here were
> reused in v2, and because how a design comes to rest on an unverified
> assumption for two years is worth being able to re-read.
