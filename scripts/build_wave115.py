"""Build the repaired self-contained Wave 115 three-arm T4 Q8 head-to-head notebook."""
...
OUTPUT = ROOT / "notebooks/glcuda_t4_wave115_h2h_q8.ipynb"
...
bootstrap = bootstrap.replace("Wave 111", "Wave 115").replace("WAVE111", "WAVE115")
bootstrap = bootstrap.replace("wave111", "wave115")
bootstrap = bootstrap.replace(
    "glcuda-t4-wave115-residual-fusion-results.zip",
    "glcuda-t4-wave115-h2h-q8-results.zip",
)
...
bootstrap = replace_once(
    bootstrap,
    'FINAL_ZIP = Path("/kaggle/working/glcuda-t4-wave115-h2h-q8-results.zip")\n',
    'FINAL_ZIP = Path("/kaggle/working/glcuda-t4-wave115-h2h-q8-results.zip")\n'
...
replacement = '''    parity = run([
    ...
    if build.returncode:
        raise RuntimeError("Wave 115 glbench release build failed")
    CARGO = cargo
    CARGO_ENV = cargo_env
    GLBENCH = TARGET / "release/glbench"
    PARITY = {"summary": parity_summary, "returncode": parity.returncode}
    BOOTSTRAP_OK = True
    print("WAVE115_BOOTSTRAP_OK", json.dumps(PARITY), flush=True)
'''
...
model = model.replace("Wave57-Q8-H2H", "Wave115-Q8-H2H")
...
llama_build = llama_build.replace("WORK", "ROOT").replace("wave57", "wave115")
...
tokenizer = old_prod[tokenizer_start:tokenizer_end].replace("wave57", "wave115")
...
(RESULTS / "wave115-h2h-q8.json").write_text(json.dumps(RESULT, indent=2), encoding="utf-8")
(RESULTS / "PRODUCTION_SUCCESS.json").write_text(
    json.dumps({"status": "valid", "decision": decision}, indent=2), encoding="utf-8")
archive()
print("WAVE115_RESULT", json.dumps(RESULT, indent=2), flush=True)
'''
...
notebook = {
    "cells": [
        md("# GwenLand glcuda Wave 115 — repaired Q8 head-to-head on Tesla T4\n\n"
