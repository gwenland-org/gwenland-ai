"""Build the repaired self-contained Wave 116 three-arm T4 Q8 head-to-head notebook."""
...
OUTPUT = ROOT / "notebooks/glcuda_t4_wave116_h2h_q8.ipynb"
...
bootstrap = bootstrap.replace("Wave 111", "Wave 116").replace("WAVE111", "WAVE116")
bootstrap = bootstrap.replace("wave111", "wave116")
bootstrap = bootstrap.replace(
    "glcuda-t4-wave116-residual-fusion-results.zip",
    "glcuda-t4-wave116-h2h-q8-results.zip",
)
...
bootstrap = replace_once(
    bootstrap,
    'FINAL_ZIP = Path("/kaggle/working/glcuda-t4-wave116-h2h-q8-results.zip")\n',
    'FINAL_ZIP = Path("/kaggle/working/glcuda-t4-wave116-h2h-q8-results.zip")\n'
...
replacement = '''    parity = run([
    ...
    if build.returncode:
        raise RuntimeError("Wave 116 glbench release build failed")
    CARGO = cargo
    CARGO_ENV = cargo_env
    GLBENCH = TARGET / "release/glbench"
    PARITY = {"summary": parity_summary, "returncode": parity.returncode}
    BOOTSTRAP_OK = True
    print("WAVE116_BOOTSTRAP_OK", json.dumps(PARITY), flush=True)
'''
...
model = model.replace("Wave57-Q8-H2H", "Wave116-Q8-H2H")
model = model.replace('    fail_phase("model-fetch")', '    fail("model-fetch")')
model = replace_once(
    model,
...
llama_build = llama_build.replace("WORK", "ROOT").replace("wave57", "wave116")
...
tokenizer = old_prod[tokenizer_start:tokenizer_end].replace("wave57", "wave116")
tokenizer = tokenizer.replace(
    'env={"CARGO_TARGET_DIR": str(TARGET)}',
    'env={**CARGO_ENV, "CARGO_TARGET_DIR": str(TARGET)}',
)
...
RESULT = {
    "wave": 116, "status": "production_measured", "gpu": fields,
...
(RESULTS / "wave116-h2h-q8.json").write_text(json.dumps(RESULT, indent=2), encoding="utf-8")
(RESULTS / "PRODUCTION_SUCCESS.json").write_text(
    json.dumps({"status": "valid", "decision": decision}, indent=2), encoding="utf-8")
archive()
print("WAVE116_RESULT", json.dumps(RESULT, indent=2), flush=True)
'''

production = production_prefix + "\n" + tokenizer + production_tail
production = production.replace("save_log(", "save(")
...
notebook = {
    "cells": [
        md("# GwenLand glcuda Wave 116 — repaired Q8 head-to-head on Tesla T4\n\n"
