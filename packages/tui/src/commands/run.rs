...
use gwenland_core::engine::GwenMode;
...
pub fn run_run_cmd(args: RunArgs, mode: GwenMode) {
    // Validate auto-stop early so the user gets a clear message before loading weights.
    if args.auto_stop < 10 {
        if mode.json {
            print_json_error(
                "INVALID_INPUT",
                format!("--auto-stop {} is dangerously low; minimum is 10%", args.auto_stop),
                None,
            );
            std::process::exit(1);
        }
    ...
    if args.interactive && mode.json {
        print_json_error(
            "INVALID_INPUT",
            "--json is only supported for single-shot gwen run, not --interactive",
            None,
        );
        std::process::exit(1);
    }
    ...
    if args.interactive {
        ...
        let callback = if mode.json {
            Some(&mut token_sink as &mut dyn FnMut(String))
        } else {
            None
        };

        match run_inference(&cfg, callback) {
            Ok(result) => {
                if mode.json {
                    ...
                    println!(
                        ...
                        serde_json::json!({
                            ...
                            "data": {
                                ...
                                "tokens_per_second": result.token_count as f64 / elapsed_secs,
                                "time_ms": elapsed_ms,
                                "memory_stopped": result.memory_stopped,
                            },
                            "error": null,
                        })
                    );
                }
                if result.memory_stopped && !mode.json {
                    std::process::exit(1);
                }
            }
            Err(e) => {
                if mode.json {
                    print_json_error(
                        "INFERENCE_FAILED",
                        format!("{:#}", e),
                        Some(serde_json::json!({ "model": cfg.model })),
                    );
                    std::process::exit(1);
                }
...
fn print_json_error(code: &str, message: impl Into<String>, details: Option<serde_json::Value>) {
    println!(
        "{}",
        serde_json::json!({
            "status": "error",
