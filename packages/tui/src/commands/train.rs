...
pub struct TrainArgs {
    ...
    #[arg(long = "lora-rank", alias = "lora-r", value_name = "N", global = false,
    ...
    #[arg(long, value_name = "N", global = false,
          help = "Stop native training after N optimizer steps")]
...
async fn run_native_path(
    ...
    if let Some(r) = args.lora_rank { config.lora.r = r; }
    if let Some(b) = args.batch_size { config.batch_size = b; }
    if let Some(s) = args.max_steps { config.max_steps = Some(s); }
...
fn build_train_config_from_args(args: &TrainArgs) -> Result<TrainConfig> {
    ...
    Ok(TrainConfig {
        ...
        batch_size:    args.batch_size.map(|b| b as u32).unwrap_or(defaults.batch_size as u32),
        grad_accum:    defaults.grad_accum as u32,
        learning_rate: args.lr.unwrap_or(defaults.lr),
        max_seq_len:   1024,
        lora_r:        args.lora_rank.map(|r| r as u32).unwrap_or(lora_defaults.r as u32),
