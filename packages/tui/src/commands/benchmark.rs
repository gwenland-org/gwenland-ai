...
pub fn run_benchmark_cmd(args: BenchmarkArgs, json_output: bool) {
    ...
    if json_output {
        let result = run_benchmarks(filter, None);
...
fn print_json_report(result: &BenchmarkResult) {
    let cold_start = result.cold_start.as_ref().map(|cs| {
        json!({
            "min_ms": cs.min_ms,
            "max_ms": cs.max_ms,
            "mean_ms": cs.mean_ms,
    ...
    let inference = result.inference.as_ref().map(|inf| {
        json!({
            "tokens_per_sec": inf.tokens_per_sec,
            "total_tokens": inf.total_tokens,
            "elapsed_secs": inf.elapsed_secs,
        })
    });
    let convert = result.convert.as_ref().map(|cv| {
        json!({
            "standard_ns_per_elem": cv.standard_ns_per_elem,
            "euler_ns_per_elem": cv.euler_ns_per_elem,
            "standard_stddev": cv.standard_stddev,
            "euler_stddev": cv.euler_stddev,
        })
    });
    let memory = result.memory.as_ref().map(|mem| {
        json!({
            "baseline_mb": mem.baseline_mb,
        })
    });
