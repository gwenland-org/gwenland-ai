...
//!
//! The pre-tokenizer patterns are written in terms of `\p{L}`, `\p{N}`,
//! `\p{M}` and `\p{P}`. Rust's standard library has no general-category
//! predicates: `char::is_alphabetic` is the `Alphabetic` *property* (a strict
//! superset of `\p{L}`) and `char::is_numeric` happens to be exactly `\p{N}`.
//! Substituting the former is what this crate did before, and it is the kind
...
//! overwhelming majority of real input — resolves in one array index; the
//! rest binary-searches. A flat bitset over all 1 114 112 codepoints would
//! be 136 KiB per class and thrash the cache for no gain, whereas these
//! 1298 ranges total a few kilobytes.

// \p{L}: 660 ranges (4970 codepoints newer than CPython 14.0's UCD)
// \p{M}: 310 ranges (42 codepoints newer than CPython 14.0's UCD)
// \p{N}: 137 ranges (40 codepoints newer than CPython 14.0's UCD)
// \p{P}: 191 ranges (23 codepoints newer than CPython 14.0's UCD)

const ASCII_L: u8 = 1 << 0;
const ASCII_M: u8 = 1 << 1;
const ASCII_N: u8 = 1 << 2;
const ASCII_P: u8 = 1 << 3;

/// Category bits for every ASCII codepoint, indexed directly.
const ASCII: [u8; 128] = [
    ,
];

/// `\p{L}` — 660 inclusive ranges.
static L_RANGES: [(u32, u32); 660] = [
    ...
    (0x0a722,0x0a788), (0x0a78b,0x0a7ca), (0x0a7d0,0x0a7d1), (0x0a7d3,0x0a7d3),
    (0x0a7d5,0x0a7d9), (0x0a7f2,0x0a801), (0x0a803,0x0a805), (0x0a807,0x0a80a),
...
/// `\p{M}` — 310 inclusive ranges.
static M_RANGES: [(u32, u32); 310] = [
    ...
    (0x00cf3,0x00cf3), (0x00d00,0x00d03), (0x00d3b,0x00d3c), (0x00d3e,0x00d44),
    (0x00d46,0x00d48), (0x00d4a,0x00d4d), (0x00d57,0x00d57), (0x00d62,0x00d63),
    (0x00d81,0x00d83), (0x00dca,0x00dca), (0x00dcf,0x00dd4), (0x00dd6,0x00dd6),
    (0x00dd8,0x00ddf), (0x00df2,0x00df3), (0x00e31,0x00e31), (0x00e34,0x00e3a),
    ...
    (0x00f3e,0x00f3f), (0x00f71,0x00f84), (0x00f86,0x00f87), (0x00f8d,0x00f97),
    (0x00f99,0x00fbc), (0x00fc6,0x00fc6), (0x0102b,0x0103e), (0x01056,0x01059),
    (0x0105e,0x01060), (0x01062,0x01064), (0x01067,0x0106d), (0x01071,0x01074),
    (0x01082,0x0108d), (0x0108f,0x0108f), (0x0109a,0x0109d), (0x0135d,0x0135f),
    (0x01712,0x01715), (0x01732,0x01734), (0x01752,0x01753), (0x01772,0x01773),
    (0x017b4,0x017d3), (0x017dd,0x017dd), (0x0180b,0x0180d), (0x0180f,0x0180f),
    (0x01885,0x01886), (0x018a9,0x018a9), (0x01920,0x0192b), (0x01930,0x0193b),
    (0x01a17,0x01a1b), (0x01a55,0x01a5e), (0x01a60,0x01a7c), (0x01a7f,0x01a7f),
    (0x01ab0,0x01ace), (0x01b00,0x01b04), (0x01b34,0x01b44), (0x01b6b,0x01b73),
    (0x01b80,0x01b82), (0x01ba1,0x01bad), (0x01be6,0x01bf3), (0x01c24,0x01c37),
    (0x01cd0,0x01cd2), (0x01cd4,0x01ce8), (0x01ced,0x01ced), (0x01cf4,0x01cf4),
    (0x01cf7,0x01cf9), (0x01dc0,0x01dff), (0x020d0,0x020f0), (0x02cef,0x02cf1),
    ...
    (0x0a82c,0x0a82c), (0x0a880,0x0a881), (0x0a8b4,0x0a8c5), (0x0a8e0,0x0a8f1),
    (0x0a8ff,0x0a8ff), (0x0a926,0x0a92d), (0x0a947,0x0a953), (0x0a980,0x0a983),
    ...
    (0x0fe00,0x0fe0f), (0x0fe20,0x0fe2f), (0x101fd,0x101fd), (0x102e0,0x102e0),
...
/// `\p{N}` — 137 inclusive ranges.
static N_RANGES: [(u32, u32); 137] = [
...
/// `\p{P}` — 191 inclusive ranges.
static P_RANGES: [(u32, u32); 191] = [
    (0x00021,0x00023), (0x00025,0x0002a), (0x0002c,0x0002f), (0x0003a,0x0003b),
    (0x0003f,0x00040), (0x0005b,0x0005d), (0x0005f,0x0005f), (0x0007b,0x0007b),
    (0x0007d,0x0007d), (0x000a1,0x000a1), (0x000a7,0x000a7), (0x000ab,0x000ab),
    (0x000b6,0x000b7), (0x000bb,0x000bb), (0x000bf,0x000bf), (0x0037e,0x0037e),
    (0x00387,0x00387), (0x0055a,0x0055f), (0x00589,0x0058a), (0x005be,0x005be),
    (0x005c0,0x005c0), (0x005c3,0x005c3), (0x005c6,0x005c6), (0x005f3,0x005f4),
    (0x00609,0x0060a), (0x0060c,0x0060d), (0x0061b,0x0061b), (0x0061d,0x0061f),
    (0x0066a,0x0066d), (0x006d4,0x006d4), (0x00700,0x0070d), (0x007f7,0x007f9),
    (0x00830,0x0083e), (0x0085e,0x0085e), (0x00964,0x00965), (0x00970,0x00970),
    (0x009fd,0x009fd), (0x00a76,0x00a76), (0x00af0,0x00af0), (0x00c77,0x00c77),
    (0x00c84,0x00c84), (0x00df4,0x00df4), (0x00e4f,0x00e4f), (0x00e5a,0x00e5b),
    (0x00f04,0x00f12), (0x00f14,0x00f14), (0x00f3a,0x00f3d), (0x00f85,0x00f85),
    (0x00fd0,0x00fd4), (0x00fd9,0x00fda), (0x0104a,0x0104f), (0x010fb,0x010fb),
    (0x01360,0x01368), (0x01400,0x01400), (0x0166e,0x0166e), (0x0169b,0x0169c),
    (0x016eb,0x016ed), (0x01735,0x01736), (0x017d4,0x017d6), (0x017d8,0x017da),
    (0x01800,0x0180a), (0x01944,0x01945), (0x01a1e,0x01a1f), (0x01aa0,0x01aa6),
    (0x01aa8,0x01aad), (0x01b5a,0x01b60), (0x01b7d,0x01b7e), (0x01bfc,0x01bff),
    (0x01c3b,0x01c3f), (0x01c7e,0x01c7f), (0x01cc0,0x01cc7), (0x01cd3,0x01cd3),
    (0x02010,0x02027), (0x02030,0x02043), (0x02045,0x02051), (0x02053,0x0205e),
    (0x0207d,0x0207e), (0x0208d,0x0208e), (0x02308,0x0230b), (0x02329,0x0232a),
    (0x02768,0x02775), (0x027c5,0x027c6), (0x027e6,0x027ef), (0x02983,0x02998),
    (0x029d8,0x029db), (0x029fc,0x029fd), (0x02cf9,0x02cfc), (0x02cfe,0x02cff),
    (0x02d70,0x02d70), (0x02e00,0x02e2e), (0x02e30,0x02e4f), (0x02e52,0x02e5d),
    (0x03001,0x03003), (0x03008,0x03011), (0x03014,0x0301f), (0x03030,0x03030),
    (0x0303d,0x0303d), (0x030a0,0x030a0), (0x030fb,0x030fb), (0x0a4fe,0x0a4ff),
    (0x0a60d,0x0a60f), (0x0a673,0x0a673), (0x0a67e,0x0a67e), (0x0a6f2,0x0a6f7),
    (0x0a874,0x0a877), (0x0a8ce,0x0a8cf), (0x0a8f8,0x0a8fa), (0x0a8fc,0x0a8fc),
    (0x0a92e,0x0a92f), (0x0a95f,0x0a95f), (0x0a9c1,0x0a9cd), (0x0a9de,0x0a9df),
    (0x0aa5c,0x0aa5f), (0x0aade,0x0aadf), (0x0aaf0,0x0aaf1), (0x0abeb,0x0abeb),
    (0x0fd3e,0x0fd3f), (0x0fe10,0x0fe19), (0x0fe30,0x0fe52), (0x0fe54,0x0fe61),
    (0x0fe63,0x0fe63), (0x0fe68,0x0fe68), (0x0fe6a,0x0fe6b), (0x0ff01,0x0ff03),
    (0x0ff05,0x0ff0a), (0x0ff0c,0x0ff0f), (0x0ff1a,0x0ff1b), (0x0ff1f,0x0ff20),
    (0x0ff3b,0x0ff3d), (0x0ff3f,0x0ff3f), (0x0ff5b,0x0ff5b), (0x0ff5d,0x0ff5d),
    (0x0ff5f,0x0ff65), (0x10100,0x10102), (0x1039f,0x1039f), (0x103d0,0x103d0),
...
/// `\p{L}` — Unicode general category L (letter).
#[inline]
pub fn is_letter(c: char) -> bool {
    let u = c as u32;
    if u < 128 {
        return ASCII[u as usize] & ASCII_L != 0;
    }
    in_ranges(&L_RANGES, u)
}

/// `\p{M}` — Unicode general category M (combining mark).
#[inline]
pub fn is_mark(c: char) -> bool {
    let u = c as u32;
    if u < 128 {
        return ASCII[u as usize] & ASCII_M != 0;
    }
    in_ranges(&M_RANGES, u)
}

/// `\p{N}` — Unicode general category N (number).
#[inline]
pub fn is_number(c: char) -> bool {
    let u = c as u32;
    if u < 128 {
        return ASCII[u as usize] & ASCII_N != 0;
    }
    in_ranges(&N_RANGES, u)
}

/// `\p{P}` — Unicode general category P (punctuation).
#[inline]
pub fn is_punctuation(c: char) -> bool {
    let u = c as u32;
    if u < 128 {
        return ASCII[u as usize] & ASCII_P != 0;
    }
    in_ranges(&P_RANGES, u)
}
