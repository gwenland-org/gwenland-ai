//! Wave 48 direct gate for register-resident compensated-Q attention.
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        return Err("Wave 48 direct gate requires a CUDA device".into());
    }
    let cuda = Cuda::probe()?;
    if (cuda.info.sm_major, cuda.info.sm_minor) != (7, 5) {
        return Err(format!(
            "Wave 48 direct gate requires sm_75, got sm_{}{}",
    ...
    if retained_blocks != 3 || candidate_blocks < 4 {
        return Err(format!(
            "Wave 48 residency gate failed: retained={retained_blocks}, candidate={candidate_blocks}"
        )
        .into());
    }

    let q = values(NTOK * WIDTH, 48);
    let k = values(N_KV * HEAD_STRIDE, 49);
    let v = values(N_KV * HEAD_STRIDE, 50);
    ...
    let first_mismatch = retained_host
        .iter()
        .zip(&candidate_host)
        .position(|(left, right)| left.to_bits() != right.to_bits());
    if let Some(index) = first_mismatch {
        return Err(format!("Wave 48 bit-exact gate failed at output index {index}").into());
    }
    ...
    println!(
        "[wave48-direct] {{\"ntok\":{NTOK},\"heads\":{N_HEADS},\"kv_heads\":{N_KV},\"head_dim\":{HEAD_DIM},\"warmup\":{WARMUP},\"iters\":{ITERS},\"rounds\":{ROUNDS},\"retained_us\":{retained_us:.3},\"candidate_us\":{candidate_us:.3},\"speedup\":{speedup:.4},\"bit_exact\":true,\"retained_dynamic_shared\":{retained_shared},\"candidate_dynamic_shared\":{candidate_shared},\"retained_blocks_per_sm\":{retained_blocks},\"candidate_blocks_per_sm\":{candidate_blocks}}}"
    );
    buffer.free(&cuda)?;
    if speedup < MIN_DIRECT_SPEEDUP {
        return Err(format!(
            "Wave 48 direct speed gate failed: {speedup:.4}x < {MIN_DIRECT_SPEEDUP:.2}x"
        )
        .into());
    }
    Ok(())
