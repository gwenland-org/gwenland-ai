//! Wave 93 direct T4 gate for the N16 register-prefetch candidate.
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        println!("[wave93-n16-prefetch] no CUDA device; nothing measured");
        return Ok(());
    }
    let cuda = Cuda::probe()?;
    if (cuda.info.sm_major, cuda.info.sm_minor) != (7, 5) {
        return Err("Wave 93 requires an SM75 device".into());
    }
    let kernels = KernelSet::load(&cuda)?;
    if !kernels.gemm_n16_prefetch_enabled(OUT_DIM as u32, IN_DIM as u32, NTOK as u32) {
        return Err(
            "set GLCUDA_GRID2D=1 GLCUDA_NTILE128=1 GLCUDA_BSTAGE=1 \
             GLCUDA_GEMM_N16=1 GLCUDA_GEMM_N16_PREFETCH=1"
                .into(),
        );
    }
    if kernels.gemm_n16_uses_m32(OUT_DIM as u32, NTOK as u32) {
        return Err("Wave 93 production shape unexpectedly selected M32".into());
    }

    let retained_resource = kernels
        .wave27_n16_resource_kernel()
        .ok_or("retained N16 resource kernel unavailable")?;
    let candidate_resource = kernels
        .wave88_n16_prefetch_resource_kernel()
        .ok_or("Wave 88 resource kernel unavailable")?;
    ...
    let (tiled_qs, tiled_scales) =
        q8_0_soa_to_bstage(&qs, &scales, OUT_DIM, IN_DIM)?;
    ...
    println!(
        "[wave93-resource] {{\"retained_kernel\":\"gl_gemm_mma_q8_bstage_n16\",\
\"candidate_kernel\":\"gl_gemm_mma_q8_bstage_n16_prefetch\",\
...
\"retained_active_blocks_per_sm\":{},\"candidate_active_blocks_per_sm\":{}}}",
    ...
    println!(
        "[wave93-direct] {{\"warmup\":{},\"iters\":{},\"repeats\":{},\
\"out_dim\":{},\"in_dim\":{},\"ntok\":{},\
...
\"bit_exact\":true,\"first_mismatch\":-1,\"gate\":{:.2},\"pass\":{}}}",
    ...
    if speedup < DIRECT_GATE {
        return Err(format!(
            "direct speed gate failed: {speedup:.4}x < {DIRECT_GATE:.2}x"
        )
        .into());
    }
    Ok(())
