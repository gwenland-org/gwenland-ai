//! Wave 109 direct T4 gate for occupancy-neutral N16/M32 prefetch.
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        println!("[wave109-n16-m32-prefetch-remat] no CUDA device; nothing measured");
        return Ok(());
    }
    let cuda = Cuda::probe()?;
    if (cuda.info.sm_major, cuda.info.sm_minor) != (7, 5) {
        return Err("Wave 109 requires an SM75 device".into());
    }
    let kernels = KernelSet::load(&cuda)?;
    if !kernels.gemm_n16_m32_prefetch_remat_enabled(OUT_DIM as u32, IN_DIM as u32, NTOK as u32) {
        return Err("set GLCUDA_GRID2D=1 GLCUDA_NTILE128=1 GLCUDA_BSTAGE=1 \
             GLCUDA_GEMM_N16=1 GLCUDA_GEMM_N16_M32_PREFETCH_REMAT=1"
            .into());
    }
    ...
    let candidate_resource = kernels
        .wave109_n16_m32_prefetch_remat_resource_kernel()
        .ok_or("Wave 109 resource kernel unavailable")?;
    ...
    if retained_active < 4 || candidate_active < 4 {
        return Err(format!(
    ...
    let candidate = |out| {
        kernels.gemm_mma_q8_bstage_n16_m32_prefetch_remat(
            &cuda,
            wqs,
            wsc,
            xqs,
    ...
    println!(
        "[wave109-resource] {{\"retained_kernel\":\"gl_gemm_mma_q8_bstage_n16_m32\",\
\"candidate_kernel\":\"gl_gemm_mma_q8_bstage_n16_m32_prefetch_remat\",\
...
\"retained_active_blocks_per_sm\":{},\"candidate_active_blocks_per_sm\":{}}}",
    ...
    println!(
        "[wave109-direct] {{\"warmup\":{},\"iters\":{},\"repeats\":{},\
\"out_dim\":{},\"in_dim\":{},\"ntok\":{},\
