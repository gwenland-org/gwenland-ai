...
/// Reorder row-major Q8_0 SoA streams into the Wave 12 prefill-only B tile.
///
/// The retained decode image remains untouched.  This duplicate cold-path
/// image is grouped as `[n_tile, k_block, row_in_tile, k_in_block]` for qs
/// and `[n_tile, k_block, row_in_tile]` for f16 scales.  A CTA can therefore
/// cooperatively stage 64 or 128 output rows for one K32 block while keeping
/// the exact original quant bytes and scale bits.  The final N tile is padded
/// with zero bytes/bits so inactive warps have an in-bounds source.
pub fn q8_0_soa_to_bstage(
    qs: &[u8],
    scales: &[u8],
    out_dim: usize,
    in_dim: usize,
) -> Result<(Vec<u8>, Vec<u8>), GlError> {
    const N_TILE: usize = 128;
    if in_dim == 0 || !in_dim.is_multiple_of(32) {
        return Err(GlError::Parse(format!(
            "Wave 12 B-stage requires non-zero in_dim divisible by 32, got {in_dim}"
        )));
    }
    let nb = in_dim / 32;
    if qs.len() != out_dim * in_dim || scales.len() != out_dim * nb * 2 {
        return Err(GlError::Parse(format!(
            "Wave 12 B-stage Q8_0 length mismatch: qs={} scales={} expected qs={} scales={}",
            qs.len(),
            scales.len(),
            out_dim * in_dim,
            out_dim * nb * 2,
    ...
    let n_tiles = out_dim.div_ceil(N_TILE);
    let mut tiled_qs = vec![0u8; n_tiles * nb * N_TILE * 32];
    let mut tiled_scales = vec![0u8; n_tiles * nb * N_TILE * 2];
    for tile in 0..n_tiles {
        for kb in 0..nb {
            let tile_qs = (tile * nb + kb) * N_TILE * 32;
            let tile_sc = (tile * nb + kb) * N_TILE * 2;
            for row in 0..N_TILE {
                let global_row = tile * N_TILE + row;
                if global_row >= out_dim {
                    continue;
                }
                let src_qs = global_row * in_dim + kb * 32;
                let dst_qs = tile_qs + row * 32;
                tiled_qs[dst_qs..dst_qs + 32].copy_from_slice(&qs[src_qs..src_qs + 32]);
                let src_sc = (global_row * nb + kb) * 2;
                let dst_sc = tile_sc + row * 2;
                tiled_scales[dst_sc..dst_sc + 2].copy_from_slice(&scales[src_sc..src_sc + 2]);
            }
        }
    }
    Ok((tiled_qs, tiled_scales))
}
...
mod tests {
    ...
    #[test]
    fn q8_0_bstage_preserves_every_real_byte_and_zero_pads_tail() {
        let out = 130usize;
        let input = 64usize;
        let nb = input / 32;
        let qs: Vec<u8> = (0..out * input).map(|i| i.wrapping_mul(29) as u8).collect();
        let scales: Vec<u8> = (0..out * nb * 2)
            .map(|i| i.wrapping_mul(17) as u8)
            .collect();
        let (tq, ts) = q8_0_soa_to_bstage(&qs, &scales, out, input).unwrap();

        for row in 0..out.div_ceil(128) * 128 {
            for kb in 0..nb {
                let tile = row / 128;
                let local = row % 128;
                let qoff = ((tile * nb + kb) * 128 + local) * 32;
                let soff = ((tile * nb + kb) * 128 + local) * 2;
                if row < out {
                    assert_eq!(
                        &tq[qoff..qoff + 32],
                        &qs[row * input + kb * 32..row * input + kb * 32 + 32]
                    );
                    assert_eq!(
                        &ts[soff..soff + 2],
                        &scales[(row * nb + kb) * 2..(row * nb + kb) * 2 + 2]
                    );
                } else {
                    assert_eq!(&tq[qoff..qoff + 32], &[0u8; 32]);
                    assert_eq!(&ts[soff..soff + 2], &[0u8; 2]);
                }
            }
        }
    }
