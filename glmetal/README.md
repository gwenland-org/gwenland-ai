...
There is no Metal backend. There are no kernels, no device memory management,
...
stub here means adding a real Metal backend later is **one match arm**, with the
...
```bash
cargo test -p glmetal   # 1 test: stub_reports_unavailable
```
